...
이 문서는 `greek-risk-analysis` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: Greek 리스크 분석 (Greek Risk Analysis)
- 연구 카테고리: Derivatives Pricing & Volatility
- 핵심 질문: 기초자산과 변동성이 동시에 움직이면 보유 옵션 포지션 손익은 어떤 Greek에 가장 민감한가?
...
- Black-Scholes 기반 Delta, Gamma, Vega 계산 구조
- 옵션 가격 민감도를 리스크 노출로 해석하는 방식
- 시나리오 충격을 손익 근사로 연결하는 흐름
